...
const WINDOW_DAYS = 7
const REVENUE_WEIGHT = 0.7
const ACTIVITY_WEIGHT = 0.3
const MIN_STAKE = 100000 // cents (KES 1,000) minimum window volume to be ranked
...
export function rankGames(stats: Record<OrderGame, GameStat>): OrderGame[] {
  const games = ORDER_GAMES.filter(g => stats[g] && stats[g].staked >= MIN_STAKE)
  ...
  const score = (g: OrderGame) =>
    REVENUE_WEIGHT * norm(revOf(g), minRev, maxRev) +
    ACTIVITY_WEIGHT * (maxAct > 0 ? stats[g].staked / maxAct : 0)

  return [...games].sort((a, b) => score(b) - score(a))
}

async function fetchStats(): Promise<Record<OrderGame, GameStat>> {
...
export async function getGameOrder(): Promise<OrderGame[]> {
  ...
  const order = rankGames(await fetchStats())
