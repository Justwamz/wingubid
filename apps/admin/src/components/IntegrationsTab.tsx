...
import { RefreshCw, Plus, Trash2, ChevronDown, ChevronUp, MessageSquare, Mail } from 'lucide-react'
...
function EmailProviderCard() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const { data } = await apiFetch<{ config: { enabled: boolean; config: Record<string, string> } }>('/admin/email-config')
    if (data) {
      setEnabled(data.config.enabled)
      setProvider(data.config.config.provider === 'sendgrid' ? 'sendgrid' : 'resend')
      setMaskedKey(data.config.config.apiKey ?? '')
      setFromEmail(data.config.config.fromEmail ?? '')
      setToEmail(data.config.config.toEmail ?? 'withdrawals@wingubet.com')
      setApiKey('')
    }
    setLoading(false)
  }, [])
  ...
  async function handleSave(e: React.FormEvent) {
    e.preventDefault()
    setSaving(true); setError(null); setSaved(false)
    const config: Record<string, string> = { provider, fromEmail: fromEmail.trim(), toEmail: toEmail.trim() }
    if (apiKey.trim()) config.apiKey = apiKey.trim()  // omit when unchanged so we don't clobber the stored key
    const { error: err } = await apiFetch('/admin/email-config', {
      method: 'PUT',
      body: JSON.stringify({ enabled, config }),
    })
    setSaving(false)
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-semibold">Email (withdrawal alerts)</h2>
        {enabled ? (
          <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-cyan-900/40 text-cyan-400 border border-cyan-700/40">Live</span>
        ) : (
          <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-gray-800 text-gray-500 border border-gray-700">Disabled</span>
      ...
      <p className="text-xs text-gray-500">
        Sends an alert on every withdrawal and each approval decision. While disabled, emails are simulated (logged) and nothing is sent.
      </p>
      ...
      <form onSubmit={handleSave} className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-3 max-w-md">
        ...
        <div>
          ...
          <input
            ...
            value={apiKey}
            onChange={e => setApiKey(e.target.value)}
            placeholder={maskedKey ? `Saved: ${maskedKey} - type to replace` : 'Provider API key'}
            className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-cyan-600 font-mono"
          />
        </div>
        <div>
          <label className="text-xs text-gray-400 block mb-1">From email</label>
          <input
            type="email"
            value={fromEmail}
            onChange={e => setFromEmail(e.target.value)}
            placeholder="alerts@wingubet.com (verified sender)"
            className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-cyan-600"
          />
        </div>
        <div>
          <label className="text-xs text-gray-400 block mb-1">Notify email</label>
          <input
            type="email"
            value={toEmail}
            onChange={e => setToEmail(e.target.value)}
            placeholder="withdrawals@wingubet.com"
            className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-cyan-600"
          />
        </div>
        ...
        <label className="flex items-center gap-2.5 cursor-pointer text-sm text-gray-300 pt-1">
          <input type="checkbox" checked={enabled} onChange={e => setEnabled(e.target.checked)} className="h-4 w-4 accent-cyan-400" />
          <span>Enabled - send live emails</span>
        </label>
